...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	// 被动 image_gen namespace 不应触发生图权限、并发槽位或 Responses 能力要求；
	// 但实际的自然语言生图请求仍需按生图请求处理。
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	// 与 HTTP Responses 路径保持一致：被动 image_gen namespace 不算生图请求，
	// 但实际执行生图的模型、tool_choice 或用户请求仍需受权限和并发控制。
